...
const AuthProvider = ({ children }) => {
  ...
  const [excelArr, setExcelArr] = useState([])
   const Login = async (uname, pass) => {
    await fetch(`http://server.appsstaging.com:4321/api/signIn`, {
      method: "POST",
      body: JSON.stringify({
        "username": uname,
        "password": pass
  ...
  const bookSlot = async (building, room, SubName, sec, corsins, sttime, entime, value) => {
    await fetch(`http://server.appsstaging.com:4321/api/addSlot`, {
      method: "POST",
      body: JSON.stringify({
  ...
  const viewSlots = async () => {
    fetch(`http://server.appsstaging.com:4321/api/listOfSlots`, {
      method: "GET",
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
  ...
  const deleteSlot = async (_id) => {
    ...
    await fetch(`http://server.appsstaging.com:4321/api/deleteSlot`, {
      method: "POST",
      body: JSON.stringify({
        "slot_id": _id,
      }),
  ...
  const BookedSlots = async (room) => {
     await fetch(`http://server.appsstaging.com:4321/api/classList`, {
      method: "POST",
      body: JSON.stringify({
        "class_name": room,
      }),
  ...
  const updateSlot = async (_id, sub, sec, teacher, day, st_time, en_time, building_name, class_name) => {
    await fetch(`http://server.appsstaging.com:4321/api/updateSlot`, {
      method: "POST",
      body: JSON.stringify({
        "slot_id": _id,
